...
class addViewController:UIViewController,NVActivityIndicatorViewable, UIGestureRecognizerDelegate,UITextFieldDelegate,addViewUpdt,UIScrollViewDelegate {
...
//        self.productTags.customBackgroundColor = UIColor(red: 145/255, green: 111/255, blue: 239/255, alpha: 1.0)
    ...
    func tagtxtedit()
    {
        self.noTagTxtClickedCnstr.constant = 150
    ...
    func tagtxtClicked()
    {
        self.noTagTxtClickedCnstr.constant = 490
        self.tagsListedCnstr.constant = 350
    ...
    {
        self.noTagTxtClickedCnstr.constant = 490
        self.tagsListedCnstr.constant = 399.5
        self.searchContainer.isHidden = false
        self.SlctdTagTitle.isHidden = false
        self.testContainer.isHidden = false
        if(self.SlctdTagsArry.count > 0)
        {
            self.SlctdTagTitle.text = "Picked Tags"
        }
        else
        {
            self.SlctdTagTitle.text = "No Tags Picked !"
        }
    }
    @objc final private func yourHandler(textField: UITextField) {
        print("Text changed")
        let centerOffsetX = (self.addIncExpScroll.contentSize.width - self.addIncExpScroll.frame.size.width) / 2
        let centerOffsetY = (self.addIncExpScroll.contentSize.height - self.addIncExpScroll.frame.size.height) / 2
        let centerPoint = CGPoint(x: centerOffsetX, y: centerOffsetY)
        self.addIncExpScroll.setContentOffset(centerPoint, animated: true)
    ...
    override func viewDidLoad() {
        ...
        addDataViewMdlObj.addViewDlgt = self
        DispatchQueue.global().async {
            self.addDataViewMdlObj.getTags()
            //self.addDataViewMdlObj.getDefltTags()
        }
    ...
    func setControlsforAdding()
     {
         ...
         {
             allTags = TKCollectionView(tags: AddDataViewModel.incmTagArry,
                                        action: .addTag,
                                        receiver: productTags)
             self.themeColor =  UIColor(red: 145/255, green: 111/255, blue: 239/255, alpha: 1.0)
             self.addTopTitle.text = "Add Income"
             ...
             self.searchContainer.borderWidth = 1
             self.searchContainer.layer.cornerRadius = 8
             self.searchContainer.borderColor = ColorManager.incomeColor()
             
         }
         else
         {
             self.themeColor =  UIColor(red: 255/255, green: 138/255, blue: 138/255, alpha: 1.0)
             allTags = TKCollectionView(tags: AddDataViewModel.expnsTagArry,
                                        action: .addTag,
                                        receiver: productTags)
             self.addTopTitle.text = "Add Expense"
             self.addamountTxtTitle.text = "Expense Amount"
             self.addbutton.setTitle("Add Expense", for: .normal)
             self.addbutton.backgroundColor = self.themeColor
             self.searchContainer.borderWidth = 1
             self.searchContainer.layer.cornerRadius = 8
             self.searchContainer.borderColor = ColorManager.expenseColor()
...
//         self.crncyTbl.borderWidth = 1
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        {
           // self.noTagTxtClickedCnstr.constant = 100
            if(textField.text?.count == 0)
            {
//                self.addIncExpScroll.setContentOffset(CGPointMake(0, max(self.addIncExpScroll.contentSize.height - self.addIncExpScroll.bounds.size.height, 0) ), animated: true)
//                let centerOffsetX = (self.addIncExpScroll.contentSize.width - self.addIncExpScroll.frame.size.width) / 2
//                let centerOffsetY = (self.addIncExpScroll.contentSize.height - self.addIncExpScroll.frame.size.height) / 2
//                let centerPoint = CGPoint(x: centerOffsetX, y: centerOffsetY)
//                self.addIncExpScroll.setContentOffset(centerPoint, animated: true)
                self.addIncExpScroll.scrollToBottom()
                self.tagtxtedit()
            }
    ...
    @IBAction func clkCrncy()
    {
        ...
        //self.crncyTblView.isHidden = false
        self.addDataViewMdlObj.getCurrency()
        self.addDataViewMdlObj.addViewDlgt = self
    ...
    @IBAction func clkAccount()
    {
        ...
        self.view.endEditing(true)
        //self.crncyTblView.isHidden = false
        self.addDataViewMdlObj.getAcnts()
        self.addDataViewMdlObj.addViewDlgt = self
        self.crncyTbl.dataSource = addDataViewMdlObj
        self.crncyTbl.delegate = addDataViewMdlObj
    }
    ...
    {
        self.addDataViewMdlObj.validateIncmExpsAddiotion(amnt: self.amountTxt.text!, descrptn: self.descrpnTxt.text!, acnt: self.acntIdVal, date: self.dateTxt.text!, tag: self.SlctdTagsArry,typeVal:self.IncExpTypVal,userTags:self.SlctdUserTagsArry)
...
extension addViewController: TKCollectionViewDelegate {
    ...
    func tagIsBeingRemoved(name: String?) {
        print("removed \(name!)")
        var tagsAry = AddDataViewModel.incmTagArry
        var tagsIdAry = AddDataViewModel.incmTagIdArry
//        var tagsAry = UserDefaults.standard.value(forKey: "incDefltTags")! as! [String]
//        var tagsIdAry = UserDefaults.standard.value(forKey: "incDefltIdTags")! as! [String]
        ...
        if(self.SlctdUserTagsArry.count == 0)
        {
            self.SlctdTagTitle.text = "No Tags picked !"
        }
        self.view.endEditing(true)
