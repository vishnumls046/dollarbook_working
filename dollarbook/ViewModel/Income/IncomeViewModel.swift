...
class IncomeViewModel:NSObject
{
    var pagecount = 0
    var isLastPage = 0
    var incmDlgt:IncomeUIupdt?
    var transArry = [Transactions]()
    var monthArry = [String]()
    var IncmArry = [Int]()
    func getTransactions()
    {
        AF.request(APImanager.GetTransactions(),method: .post,parameters: ["user_id":UserDefaults.standard.value(forKey: "user_id")!,"transaction_type":"1", "page_no":self.pagecount],encoding: URLEncoding.default, headers:["Oakey":"INCEXPMND1254"]).responseJSON
        {
            response in
            if let data = response.data
            {
                do{
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.incmDlgt?.recentTblUpdt(trnsCnt:"\(self.transArry.count)")
                    }
                ...
                do{
                    ...
                    for anItem in monthResponse.transactions! as Array {
                      ...
                      //let tagID = anItem["tag_id"] as! String
                        ...
                        let x = nf.number(from: anItem.totalIncome!)!.intValue
                        self.IncmArry.append(x)
...
//                        }
                    ...
                    DispatchQueue.main.async {
                        self.incmDlgt?.barGrphUpt(valArry:self.IncmArry,mnthArry:self.monthArry)
                    }
...
extension IncomeViewModel: UITableViewDataSource,UITableViewDelegate {
...
//                    }
                            ...
                            DispatchQueue.main.async {
                                self.incmDlgt?.alerts(alrtStr: acntResponse.addresult!.message!)
                            }
