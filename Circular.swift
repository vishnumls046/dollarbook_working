...
open class Circular: UIView {
    ...
    override public func draw(_ rect: CGRect) {
        ...
        for i in 0..<_percentages.count {
            ...
            let endAngle = startAngle + ( _percentages[i] * 1.8 ) - 4
            
            let shapeLayer =   self.addArac(with: _colors[i], in: rect, startAngle: startAngle, endAngle: endAngle,cnt:i)
    ...
    private func showPercentages(midAngel:Double, percentage:Double) {
        ...
        UIView.animate(withDuration: 0.5, delay: delay, options: .curveEaseOut, animations: {
            percentageLabel.alpha = 0   // To on or off percentage label
        })
    }
    ...
    private func addArac(with color:UIColor ,in rect:CGRect, startAngle:Double , endAngle:Double,cnt:Int)-> CAShapeLayer {
        if(cnt == 0)
        {
